...
while (<STDIN>) {
   ...
   if ($raw) {
      ...
      if ($features[$#features] ne "o") { 
         $features[$#features] = "b_$features[$#features]";
      }
      if ($features[$#features-1] ne "o") { 
         $features[$#features-1] = "b_$features[$#features-1]";
      }
   }
   # 20040126 ET code which allows hyphens in the types
   if ($features[$#features] =~ /^([^_]*)_(.*)$/) {
      $guessed = $1;
      $guessedType = $2;
   } else { 
...
#  ($correct,$correctType) = split(/-/,pop(@features));
   ...
   if ($inCorrect) {
      ...
      # printf "\n";
      # printf "in!!\n";
      # printf &endOfChunk($lastCorrect,$correct,$lastCorrectType,$correctType);
      # printf &endOfChunk($lastGuessed,$guessed,$lastGuessedType,$guessedType);
      # printf "out!\n";
